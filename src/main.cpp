...
const string strMessageMagic = "GoldRushCoin Signed Message:\n";
...
int CMerkleTx::GetBlockIndex(CBlockIndex* &pindexRet) const
{
    if (hashBlock == 0 || nIndex == -1)
        return 0;
...
static const int CUTOFF_HEIGHT = 127000;	// Height at the end of 88 days or ~ 2.5M  Coins
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash, unsigned int nTime)
{
	...
	else if (  (nHeight >= 500 ) &&  (nHeight < 720 ) )
	{
		nSubsidy = 5 * COIN;
	}
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
    ...
    if(nHeight < YEARLY_BLOCKCOUNT)
        ...
        nRewardCoinYear = 0.8 * MAX_MINT_PROOF_OF_STAKE;

    else if(nHeight > (3 * YEARLY_BLOCKCOUNT))
        nRewardCoinYear = 0.4 * MAX_MINT_PROOF_OF_STAKE;

    else if(nHeight > (4 * YEARLY_BLOCKCOUNT))
        nRewardCoinYear = 0.2 * MAX_MINT_PROOF_OF_STAKE;

    else if(nHeight > (5 * YEARLY_BLOCKCOUNT))
        nRewardCoinYear = 0.1 * MAX_MINT_PROOF_OF_STAKE;

    int64 nSubsidy = nCoinAge * nRewardCoinYear / 365;

    if (fDebug)
        printf("GetPRoofOfStakeReward(): reward=%"PRI64d"", nRewardCoinYear);
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in GoldRushCoin
    bool fStrictPayToScriptHash = true; // Always active in GoldRushCoin
	...
	if (vtx[0].GetValueOut() > GetProofOfWorkReward(pindex->nHeight, nFees, prevHash, pindex->nTime))
		return false;
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    if (!CheckStakeModifierCheckpoints(pindexNew->nHeight, pindexNew->nStakeModifierChecksum))
        return error("AddToBlockIndex() : Rejected by stake modifier checkpoint height=%d, modifier=0x%016"PRI64x", 0x%016"PRI64x, pindexNew->nHeight, nStakeModifier, pindexNew->nStakeModifierChecksum);
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "GoldRushCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "11-18-2014 In all 50 states, it's below freezing in at least one spot";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1416347000;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 10836083;

        if (fTestNet)
        {
                 block.nTime    = 1416346000;
                 block.nNonce   = 1015878;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x7df92ae3c388ea58158b6647a4e731e90d1c56c38719e6365c4e403afc435298"));
...
bool LoadExternalBlockFile(FILE* fileIn)
{
    ...
    int nLoaded = 0;
    {
        ...
        try {
            ...
            unsigned int nPos = 0;file:///var/run/vmblock-fuse/blockdir/7bbd95ac/GoldRushCoin-Qt
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake)
{
    ...
    int64 nFees = 0;
    {
        ...
        if (pblock->IsProofOfWork())
            pblock->vtx[0].vout[0].nValue = GetProofOfWorkReward(pindexPrev->nHeight+1, nFees, pindexPrev->GetBlockHash(), pindexPrev->nTime);
